...
import { cacheArticle, getCachedArticle, isOnline } from "@/lib/offline";
...
export default function ArticleReader({ id }: { id: number }) {
  ...
  // ── Load article + vocab (with offline fallback) ──────────
  useEffect(() => {
    setLoading(true);
    (async () => {
      try {
        ...
        if (online) {
          art = await fetchArticle(id);
          cacheArticle(art).catch(() => {});
        } else {
          art = await getCachedArticle(id);
        }
        const vocab = online ? await fetchVocabulary(activeLang) : [];
        setArticle(art);
  ...
  const handleArticleMouseUp = useCallback((_e: React.MouseEvent<HTMLDivElement>) => {
    const sel = window.getSelection();
    ...
    setActiveWord(null);
    setSelection({ text, charStart, charEnd, x: rect.left, y: rect.top });
  }, []);
  ...
  return (
    <div className="page-container">
      <div style={{ marginBottom: "var(--space-6)" }}>
        <Link
          href="/articles"
          style={{ display: "inline-flex", alignItems: "center", gap: "var(--space-1)", fontFamily: "var(--font-sans)", fontSize: "var(--text-sm)", color: "var(--color-ink-light)", textDecoration: "none", marginBottom: "var(--space-4)", transition: "color 150ms ease" }}
        >
          <ArrowLeft size={14} /> Articles
        </Link>
        <h1 style={{ fontFamily: "var(--font-serif)", fontSize: "var(--text-xl)", fontWeight: "var(--font-semibold)", color: "var(--color-ink)", margin: "0 0 var(--space-2)", lineHeight: "var(--leading-tight)" }}>
          {article.title}
        </h1>
        <span style={{ fontFamily: "var(--font-sans)", fontSize: "var(--text-xs)", color: "var(--color-ink-light)", display: "flex", alignItems: "center", gap: "var(--space-2)" }}>
          <FileText size={12} />
      ...
      >
        {paragraphs.map((para, i) => (
          <ArticleParagraph key={i} text={para} savedWords={savedWords} onWordClick={handleWordClick} />
        ))}
      </div>
